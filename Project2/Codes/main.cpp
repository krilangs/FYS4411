...
int main() {
    ...
    double E_L;
    if (interaction == true)
        E_L = 3.0;   // Analytical energy for 2 interacting particles in 2D
    else
        E_L = 0.5*numberOfParticles*numberOfDimensions;  // Analytical energy for non-interaction
    ...
    if (method == "MetropolisBruteForce"){
    ...
    if (method == "MetropolisImportance"){
        if (interaction == true){ // Interaction
            // SGD data file
