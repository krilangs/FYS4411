...
\Rightarrow\quad \nabla_k^2 P&=P\nabla_k g\cdot(\nabla_k P)^2+Pg\nabla_k^2 P+g(\nabla_k P)^2
\end{align*}
The terms containing first and second derivatives have to cancel each other, which only happens if $g=\frac{1}{P}$. This yields the drift force for a single article $k$:
\begin{equation}
\label{eq:F}
...
\end{align*}
Since we will use scaling and natural units, $\hbar=m=\omega_{ho}=1$, we can simplify the local energy to
\begin{equation}
...
\end{equation}
we see that the local energy becomes stable depending on the dimensionality of the system and the number of particles. This will give the ground state energy of the system in this case.
...
\label{subsect:brute_Metropolis}
\subsection{Importance Sampling}
\label{subsect:Importance}
